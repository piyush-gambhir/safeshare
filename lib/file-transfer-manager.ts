import { WebRTCService, type SignalData } from "./webrtc-service"
import { EncryptionService } from "./encryption-service"
import { FileChunker, type FileMetadata } from "./file-chunker"

export type TransferRole = "sender" | "receiver"

export type TransferState = "idle" | "preparing" | "connecting" | "transferring" | "paused" | "completed" | "error"
...
export class FileTransferManager {
  ...
  private onStateChangeCallback: ((state: TransferState) => void) | null = null
  ...
  private setupWebRTCListeners() {
    this.webrtcService.onConnectionStateChange((state) => {
      if (state === "connected") {
        ...
        if (this.state === "transferring" || this.state === "connecting") {
    ...
    this.webrtcService.onDataChannelStateChange((state) => {
      if (state === "open" && this.role === "sender" && this.state === "preparing") {
        this.startTransfer()
      }
    })
  ...
  public async initiateSend(file: File): Promise<void> {
    ...
    try {
      ...
      this.setError("initialization", `Failed to initiate send: ${error}`)
    }
  }
  ...
  private async prepareTransfer(): Promise<void> {
    if (this.role === "sender" && this.file) {
      try {
        ...
        const setupDataBuffer = new TextEncoder().encode(setupDataString).buffer

        this.webrtcService.sendData(setupDataBuffer)
        this.startTransfer()
      } catch (error) {
        this.setError("preparation", `Failed to prepare transfer: ${error}`)
      }
    }
  }
  ...
  private async sendNextChunk(chunkId: number): Promise<void> {
    ...
    try {
      ...
      // Encrypt the chunk
      const encryptedData = await this.encryptionService.encrypt(chunk.data)
      ...
      const combinedBuffer = new Uint8Array(
        headerSizeBuffer.byteLength + headerBuffer.byteLength + encryptedData.byteLength,
      )
      combinedBuffer.set(new Uint8Array(headerSizeBuffer), 0)
      combinedBuffer.set(new Uint8Array(headerBuffer), headerSizeBuffer.byteLength)
      combinedBuffer.set(new Uint8Array(encryptedData), headerSizeBuffer.byteLength + headerBuffer.byteLength)
      ...
      this.setError("transfer", `Failed to send chunk ${chunkId}: ${error}`)
    }
  }
  ...
  private async handleReceivedData(data: ArrayBuffer): Promise<void> {
    try {
      ...
      if (header.type === "setup") {
        ...
        const chunkData = await this.encryptionService.decrypt(encryptedChunkData)
        ...
        if (this.receivedMetadata && this.chunksTransferred >= this.receivedMetadata.totalChunks) {
          this.assembleReceivedFile()
        }
      }
    } catch (error) {
      this.setError("receive", `Failed to process received data: ${error}`)
    }
  }
  ...
  private assembleReceivedFile(): void {
    ...
    try {
      ...
      const receivedFile = new File([fileData], this.receivedMetadata.name, { type: this.receivedMetadata.type })
  ...
  private updateProgress(): void {
    ...
    const totalBytes = this.role === "sender" ? this.file?.size || 0 : this.receivedMetadata?.size || 0
