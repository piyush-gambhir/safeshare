...
export class SignalingService {
  ...
  private onChannelCreatedCallback: ((channelId: string) => void) | null = null
  private onPeerConnectedCallback: ((peerId: string) => void) | null = null

  public async createChannel(): Promise<string> {
    try {
      const response = await fetch(`${SignalingService.API_ENDPOINT}/channel`, {
        method: "POST",
      })
  ...
  public async joinChannel(channelId: string): Promise<void> {
    try {
      const response = await fetch(`${SignalingService.API_ENDPOINT}/channel/${channelId}/join`, {
        method: "POST",
      })
  ...
  public async sendSignal(signal: SignalData): Promise<void> {
    ...
    try {
      const response = await fetch(`${SignalingService.API_ENDPOINT}/channel/${this.channelId}/signal`, {
        method: "POST",
        headers: {
  ...
  private async pollSignals(): Promise<void> {
    ...
    try {
      const response = await fetch(`${SignalingService.API_ENDPOINT}/channel/${this.channelId}/signals`)

      if (!response.ok) {
        throw new Error(`Failed to poll signals: ${response.statusText}`)
      }
  ...
  public closeChannel(): void {
    ...
    if (this.channelId) {
      fetch(`${SignalingService.API_ENDPOINT}/channel/${this.channelId}/close`, {
        method: "POST",
      }).catch((error) => {
